...
1、where 是在分组前对数据进行筛选，不满足条件的数据不会参数分组，where后不能跟聚合函数条件

2、group by在数据已经完成基础筛选后，进行分组

3、having 是在分组后对数据进行进一步筛选，having后可以跟聚合函数条件

4、order by 是数据已经完成筛选后更换顺序

5、limit 是最终对查询的数据的记录数进行截取
